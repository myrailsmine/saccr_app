# ==============================================================================
# UNIFIED SA-CCR ENGINE - COMPLETE IMPLEMENTATION
# calculations/saccr_engine.py
# ==============================================================================

"""
Unified SA-CCR calculation engine with all 24 steps in a single class.
Combines ComprehensiveSACCREngine and SACCRStepCalculator for simplicity.
"""
...
from models.enums import AssetClass, TradeType, CollateralType, DataQualityIssueType, DataQualityImpact
...
from config.regulatory_params import (
    SUPERVISORY_FACTORS, SUPERVISORY_CORRELATIONS, COLLATERAL_HAIRCUTS, RISK_WEIGHT_MAPPING
)
from config.settings import G10_CURRENCIES, BASEL_ALPHA, BASEL_CAPITAL_RATIO


@dataclass
class CalculationStep:
    """Represents a single calculation step with thinking process."""
    step: int
    title: str
    description: str
    data: Dict[str, Any]
    ...
    def add_thinking(self, reasoning: str, key_insight: str = "", formula_detail: str = ""):
        """Add thinking process to the step."""
        self.thinking = {
            'step': self.step,
            'title': f"{self.title} Analysis",
            'reasoning': reasoning,
            'formula': formula_detail or self.formula,
...
class UnifiedSACCREngine:
    ...
    def calculate_comprehensive_saccr(self, netting_set: NettingSet, 
...
• It forms the primary input for the PFE calculation, which will then be scaled by the multiplier.
    ...
    def _step14_sum_v_c_enhanced(self, netting_set: NettingSet, 
        ...
        position_desc = ('Out-of-the-money (favorable)' if sum_v < 0 else 
                        'In-the-money (unfavorable)' if sum_v > 0 else 
